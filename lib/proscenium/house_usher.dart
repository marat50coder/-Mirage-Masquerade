...
class HouseUsher {
  ...
  final PlaybillScout courier;
  final BoxOffice ledger;
  final Callboy herald;
  final FootlightAgent agent;
  final bool runtimeEnabled;

  bool get enabled => runtimeEnabled && HouseBrief.houseCredentialsReady;
  ...
  Future<StageTarget> _decide({
    ...
    if (!enabled) {
      _trace(() => '[MM.HOUSE] gate closed runtime=$runtimeEnabled '
          'creds=${HouseBrief.houseCredentialsReady}');
      onProgress(1);
      return const HouseTarget();
    }

    _trace(() => '[MM.HOUSE] decide route=${vault.route}');
    ...
    final coldUrl = await DropCue.consume();
    if (coldUrl != null) {
      await vault.saveRoute(HouseRoute.mirror);
      await vault.consumePushUrl();
      unawaited(_backgroundDispatch());
      onProgress(1);
      return PaneTarget(coldUrl, coldLaunch: true);
    }
    ...
    final pushBoot = await DropCue.consumeBoot();
    if (pushBoot && vault.route != HouseRoute.mirror) {
      _trace(() => '[MM.HOUSE] cold push tap w/o URL → force mirror');
      await vault.saveRoute(HouseRoute.mirror);
    }

    // Truly-offline fast path — jumps to QuietWing instantly instead of
    ...
    if (await scout.isConfirmedOffline()) {
      _trace(() => '[MM.HOUSE] confirmed offline → hush');
      onProgress(1);
      return const QuietTarget();
    }

    onProgress(0.14);
    return switch (vault.route) {
      HouseRoute.undecided => _firstDecision(onProgress),
      HouseRoute.mirror => _returningMirror(onProgress),
      HouseRoute.house => _returningHouse(onProgress),
    };
  }
  ...
  Future<StageTarget> _firstDecision(void Function(double) progress) async {
    ...
    if (pushUrl != null && pushUrl.isNotEmpty) {
      _trace(() => '[MM.HOUSE] first: FCM initial push → open');
      await vault.saveRoute(HouseRoute.mirror);
      unawaited(_backgroundDispatch());
      progress(1);
      return PaneTarget(pushUrl, coldLaunch: true);
    }
    ...
    // which used to strand the very first launch on QuietWing. The config
    // POST below is the authoritative online test; if it fails without any
    // server response we route to a retryable QuietTarget further down.
    progress(0.5);
    ...
    if (!reply.serverResponded) {
      _trace(() => '[MM.HOUSE] first: config network fail → hush (retryable)');
      return const QuietTarget();
    }
    ...
    if (hadAttribution) {
      await vault.saveRoute(HouseRoute.house);
    }
    return const HouseTarget();
  }
  ...
  Future<StageTarget> _returningMirror(void Function(double) progress) async {
    ...
    if (pendingPush != null && pendingPush.isNotEmpty) {
      _trace(() => '[MM.HOUSE] returning-mirror: FCM push → open');
      progress(1);
      return PaneTarget(pendingPush, coldLaunch: true);
    }
    ...
    progress(1);
    if (reply.hasDestination) return PaneTarget(reply.url!);
    if (cached != null) return PaneTarget(cached);
    // Truly offline (POST failed) — Hush, retryable.
    if (!reply.serverResponded) return const QuietTarget();
    return const QuietTarget();
  }
  ...
  Future<StageTarget> _returningHouse(void Function(double) progress) async {
    ...
    final pushUrl = await vault.consumePushUrl();
    if (pushUrl != null && pushUrl.isNotEmpty) {
      _trace(() => '[MM.HOUSE] returning-house: FCM push → open');
      await vault.saveRoute(HouseRoute.mirror);
      unawaited(_backgroundDispatch());
      progress(1);
      return PaneTarget(pushUrl, coldLaunch: true);
    ...
    if (!_bodyHasAttribution(body)) {
      await vault.saveRoute(HouseRoute.undecided);
    }
    return const HouseTarget();
  }
  ...
  Future<(HouseReply reply, Map<String, dynamic> body)> _requestConfig({
    String? token,
  }) async {
    ...
    final bool forceMirror = kDebugMode
        ? HouseBrief.debugForceMirror
        : HouseBrief.forceMirrorRelease;
    if (forceMirror) {
      ...
      HouseBrief.debugMirrorParams.forEach((key, value) {
        body[key] = value;
      });
      _trace(() => '[MM.HOUSE] force mirror: af_status=Non-organic '
          '(release=${!kDebugMode})');
    }
    final reply = await ledger.request(body);
    _trace(() => '[MM.HOUSE] cfg af_status=${body['af_status']} '
  ...
  Future<void> _refreshForToken(String token) async {
    ...
    if (_decideFuture != null) {
      _trace(() => '[MM.HOUSE] token refresh skipped (decide in flight)');
      return;
    }
    ...
    if (cached != null && cached.isNotEmpty && !vault.cachedUrlExpired) {
      _trace(() => '[MM.HOUSE] token refresh skipped (cache still fresh)');
      return;
    }
    try {
      await courier.awaitSignals();
